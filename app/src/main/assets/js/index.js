...
Graph.Node = function(id, node){
    ...
    node.id = id;
    node.edges = [];
    node.tipoTarefa = node.tipoTarefa;
    node.hide = function() {
        this.hidden = true;
    ...
    drawNode: function(node) {
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                var ellipse;
                if(node.tipoTarefa == "inicia"){
                    ellipse = r.ellipse(0, 0, 30, 20).attr({fill: "#bf0000", stroke: color, "stroke-width": 2});
                }else if (node.tipoTarefa == "meio"){
                    ellipse = r.ellipse(0, 0, 30, 20).attr({fill: "#bfa200", stroke: color, "stroke-width": 2});
                }else{ // finaliza
                    ellipse = r.ellipse(0, 0, 30, 20).attr({fill: "#00bfa2", stroke: color, "stroke-width": 2});
                }
